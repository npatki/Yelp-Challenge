...
Step 2 can be directly supervised because we are given the actual average rating for a business. Thus, we can tune hyperparameters to the inference problem using the same train-validation-test strategy used in class by splitting the business data into 3 categories. We train the model using the business training data, re-run the model using the validation data, and use the results of the validation data to tune any hyperparameters.
